	  ...
	  if(!flagNewMap){
		  for(uint8_t i=0;i<Ndata;i++ ){
			  if (flags[i] == 1 || currMillis-lastMillis[i] > 2000){	//al massimo ogni due secondi ogni valore si aggiorna (display)
				  ...
				  flags[i] = 0;
				  NEXTION_SendStringMix(names[i], *arrayData[i], i);
...
void NEXTION_SendStringMix (char* elemento,int valore,int index){ //tipo può essere txt o val
	char buff[90];
	int len;
	if (uart_ready){
		//uart_ready = 0;
		switch (index){
			case 1:	//temp Batteries
				if(valore > sogliaTempBatteria){
					...
					NEXTION_SendString((uint8_t*)buff,len);
					NEXTION_SendString(cmd_end,3);
					len = sprintf(buff,"t8.bco=63488");	//rosso
					NEXTION_SendString(buff, len);
					NEXTION_SendString(cmd_end, len);
				}
				else{
...
void NEXTION_SendString(uint8_t* buff,int len){
	if(uart_ready){
		uart_ready = 0;
